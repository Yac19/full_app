    ...
    stages {
        stage("Clone the content of my Project repo on GitHub") {
            steps {
                ...
                if [ -d "Microservice" ]; then rm -rf Microservice/; fi
                git clone https://github.com/Yac19/microservice-app.git
                cd Microservice
                ls
            }
        }
        stage('Build the Docker images of the microservice and jenkins') {
            steps {
                ...
                if [ -f Dockerfile_flask_app ];
                then echo "Docker file found ! Initiating the construction of the microservice image !"
                sh "docker build -t flask-app -f Dockerfile_flask_app ."
                cd ..
                cd jenkins_microservice
                if [ -f Dockerfile ];
                then echo "Docker file found ! Initiating the build of Jenkins image !"
        ...
        stage('Test') {
            steps {
                ...
                then echo "Docker compose found ! Initiating the build of al the images for Owncloud app ! "
